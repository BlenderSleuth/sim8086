...
pub fn simulate(binary_path: &Path, mut delegate: impl SimulatorDelegate, exec: bool, dump_file: Option<&str>) -> Result<String, std::io::Error> {
    ...
    while let Some(instruction) = simulator.decode() {
        ...
        if exec {
            ...
            let sim_result = simulator.simulate(&instruction, |simulator, code|{
            ...
            match sim_result {
                Ok(result) => {
                    ...
                    write!(output, " ; ip:{old_ip:#0x}->{ip:#0x} {result}").unwrap();
                    old_ip = ip;
                }
                Err(err) => match err {
                    SimulateErr::UnknownInstruction => panic!("Cannot simulate instruction: {instruction}"),
...
pub fn simple_simulate(binary_path: &Path) -> Result<String, std::io::Error> {
    simulate(&binary_path, DefaultDelegate {}, false, None)
}
...
mod tests {
    ...
    fn run_simulate_listing(listing: &str) {
        let input_listing_path = get_listing_path(listing);
        let output = simulate(&input_listing_path, DefaultDelegate {}, true, None).expect("Simulator error.");
